    ...
    int16_t x, int16_t y, uint16_t w, uint16_t h, uint8_t *bmp)
{
  ...
  ili9341_spi_tft_set_address_rect(lcd, _x, _y, _w, _h);
  ili9341_spi_tft_select(lcd);

  HAL_GPIO_WritePin(lcd->data_command_port, lcd->data_command_pin, __GPIO_PIN_SET__);

  for (int16_t j = 0; j < _h; ++j, ++y) {
    for (int16_t i = 0; i < _w; ++i) {
      if (i & 7) {
        byte <<= 1;
      } else {
        byte = bmp[(j * byteWidth) + ((_x - x) / 8) + (i / 8)];
      }

      if (byte & 0x80) {
        spi_tx_block[((j & 1) * _w) + i] = fg_le;
      } else {
        spi_tx_block[((j & 1) * _w) + i] = bg_le;
      }
    }

    ili9341_transmit_wait(lcd);
    ili9341_transmit_color(lcd, _w * 2, &(spi_tx_block[(j & 1) * _w]), ibNo);
  }
